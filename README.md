...
Edit the Education and Selected projects sections in `vitae.html`. These are ordinary HTML entries and do not automatically update when you replace the PDF. The introductory “Read my full CV (PDF)” link opens the complete document.

For an education entry, duplicate an `<article class="education-row">...</article>` block. Keep its date range in `education-date` and its school, location, and degree details in `education-details`; this keeps the dates and text aligned on all screen sizes. Update each `<time datetime="YYYY-MM">` value along with the visible date. Only the school name belongs inside `<strong>`, followed by ` · City, Province` in regular text.
